...
from common.datasets import ksdd_dataset, utils
from common.utils import convert_bbox_to_dicts, convert_mask_to_bbox

dataset = ksdd_dataset.KSDDDataset(
    "../datasets/KSDD2", resize=1024, split=utils.DatasetSplit.MATROID_TRAIN, no_square_pad=True, patches_per_row=1, patches_per_column=1, train_split_percent=0.83
)
...
with open(
    '../datasets/KSDD2/matroid_detector_v2_data_file/bbox.csv', 'w', newline=''
) as csvfile:
    writer = csv.writer(csvfile)
    for (image_name, image_path), bbox_dicts in all_bbox_dicts.items():
        image_copy_dir = '../datasets/KSDD2/matroid_detector_v2_data_file/defects'
