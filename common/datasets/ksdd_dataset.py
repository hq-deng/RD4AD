...
class KSDDDataset(torch.utils.data.Dataset):
    """
    PyTorch Dataset for ksdd_dataset.
    """
    ...
    def __getitem__(self, idx):
        ...
        if (self.split == DatasetSplit.TEST or self.split == DatasetSplit.MATROID_TRAIN) and mask_path is not None:
    ...
    def get_image_data(self):
        ...
        datapath = os.path.join(self.source, "all")
        data_to_iterate = []
        for img in os.listdir(datapath):
            if img.endswith("_GT.png"): # It's a mask
                continue
            gt = os.path.join(datapath, f'{os.path.splitext(img)[0]}_GT.png')
            gt_arr = np.array(PIL.Image.open(gt))
            if np.sum(gt_arr) == 0: # It's a non-defective image
                gt = None
